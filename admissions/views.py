...
def screening_dashboard(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                f"Status updated to {new_status.title()} for {app.full_name}",
            )
        except Exception:
...
def screening_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('admissions:screening_list')
    return render(request, 'screening_update.html', {'app': app})
...
def screening_list(request):
    applications = AdmissionApplication.objects.all().order_by('-submitted_at')
    return render(request, 'screening_list.html', {'applications': applications})
